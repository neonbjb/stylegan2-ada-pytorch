...
class SrGenerator(nn.Module):
    def __init__(self,
        ...
        self.gen_bank = Generator(z_dim, sr_enc_dim, w_dim, img_resolution, img_channels, mapping_kwargs, synthesis_kwargs)
    ...
    def do_encoder(self, lq, **synthesis_kwargs):
        ...
        return self.encoder(lq, **block_args)

    def do_latent_mapping_for_single(self, z, lq_latent, truncation_psi=1, truncation_cutoff=None,
        ...
        ws = self.gen_bank.mapping(z, lq_latent, skip_trunc_and_broadcast=True, skip_w_avg_update=skip_w_avg_update)
    ...
    def do_latent_mapping_with_mixing(self, z, lq_latent, mixing_prob, truncation_psi=1, truncation_cutoff=None):
        ws = self.do_latent_mapping_for_single(z, lq_latent, truncation_psi, truncation_cutoff)
        if mixing_prob > 0:
            ...
            ws[:, cutoff:] = self.do_latent_mapping_for_single(torch.randn_like(z), lq_latent=lq_latent,
    ...
    #    img = gen(z=<z>, lq=<lq>, lq_latent=<lq_lat>)
    # 2) Perform your own mapping and feed in the ws:
    #    enc_c = gen.do_encoder(lq=<lq>)
    #    ws = gen.do_latent_mapping_for_single(z=<z>, lq_latent=<lq_lat>)  # Or create your own ws.
    #    img = gen(ws=ws, enc_conv_outs=enc_c)
    def forward(self, z=None, c=None, lq=None, lq_latent=None, ws=None, enc_conv_outs=None, truncation_psi=1,
        ...
        assert (z is not None and lq is not None and lq_latent is not None) or (ws is not None and enc_conv_outs is not None)
        if lq is not None:
            conv_outs = self.do_encoder(lq, **synthesis_kwargs)
        else:
...
if __name__ == '__main__':
    ...
    lq_enc = torch.randn((1,1000)).to('cuda')
    gen(z, None, lq, lq_enc, truncation_psi=1, truncation_cutoff=None)
    # Also test the style mixing method
    #enc_c, enc_l = gen.do_encoder(lq=lq)
    #ws = gen.do_latent_mapping_with_mixing(z=z, lq_latent=enc_l, mixing_prob=1)
    #img = gen(ws=ws, enc_conv_outs=enc_c)
