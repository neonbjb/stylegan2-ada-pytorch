...
class Discriminator(torch.nn.Module):
    def __init__(self,
        ...
        cmap_dim = 512
        self.mapping = MappingNetwork(z_dim=0, sr_dim=sr_enc_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)

    def forward(self, img, lq_enc, **block_kwargs):
        x = None
        for res in self.block_resolutions:
            block = getattr(self, f'b{res}')
        ...
        if self.stop_training_at > 0:
            for p in self.b4.parameters():
                p.requires_grad = False
        srmap = self.mapping(None, lq_enc)
        x = self.b4(x, img, srmap)
        return x
