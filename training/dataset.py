...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        lq_scale = 8,           # Downsample factor for LQ images
    ):
        self._name = name
    ...
    def __getitem__(self, idx):
        ...
        if self._xflip[idx]:
            ...
            image = image[:, :, ::-1]
        if self._lq_scale > 1:
            lq = torch.nn.functional.interpolate(torch.from_numpy(image).float().unsqueeze(0), scale_factor=1/self._lq_scale, mode="bilinear", align_corners=False, recompute_scale_factor=False).squeeze(0).numpy()
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if resolution is not None and (raw_shape[2] != 3*resolution//4 or raw_shape[3] != resolution):
